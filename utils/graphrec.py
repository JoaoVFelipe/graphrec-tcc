...
def GraphRec(train, test,ItemData=False,UserData=False,Graph=False,Dataset='100k'):
    ...
    iter_test = OneEpochIterator([test["user"],test["item"],test["rate"]],batch_size=BATCH_SIZE)
...
def get_UserData1M():
    ...
    df = pd.read_csv('data/ml1m/users.dat', sep='::', header=None, names=col_names, engine='python')
    del df["PostCode"]
    df["user"]-=1
...
def get_data1M():
    global PERC
    df = read_process("data/ml1m/ratings.dat", sep="::")
    rows = len(df)
...
def get_dataaiv():
    global PERC
    df = read_process("data/aiv/ratings.dat", sep="::")
    rows = len(df)
    df = df.iloc[np.random.permutation(rows)].reset_index(drop=True)
    split_index = int(rows * PERC)
    df_train = df[0:split_index]
...
#Without Graph Feature w/o external features
MFSIZE=40
UW=0.08
IW=0.06
LR=0.0002
