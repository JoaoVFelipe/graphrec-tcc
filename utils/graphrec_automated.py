...
# def inferenceSparse(phase,user_batch, item_batch,uflength,iflength, user_num, item_num,UReg=0.05,IReg=0.1, MFSIZE=50):
...
def GraphRec(train, test,
    ...
    print("Parameters -----------")
    print("MFSIZE ---", MFSIZE, "UW ---", UW, "IW ---", IW, "LR ---", LR, "BATCH_SIZE ---", BATCH_SIZE, "EPOCH_MAX ---", EPOCH_MAX)
    ...
    infer, regularizer = inferenceDense(phase,user_batch, item_batch,w_user,w_item, user_num=USER_NUM, item_num=ITEM_NUM, MFSIZE=MFSIZE, UW=UW, IW=IW)
    ...
    if True:
        ...
        for i in range(EPOCH_MAX * samples_per_batch):
            ...
            if i % samples_per_batch == 0:
                ...
                ## Print only some checkpoints to save space in the log
                if((i // samples_per_batch )%20 == 0 or (i // samples_per_batch) == (EPOCH_MAX * samples_per_batch)):
